...
const getPosts = (currentPage = 1, category) =>
  IMOUMEDIA24_HTTP_REQUEST.get(ENDPOINTS.POSTS, {
    params: {page: currentPage, categories: category},
  });

const getCategories = () => IMOUMEDIA24_HTTP_REQUEST.get(ENDPOINTS.CATEGORIES);

const getAuthor = authorId =>
  IMOUMEDIA24_HTTP_REQUEST.get(`${ENDPOINTS.USERS}/${authorId}`);

const getImage = path => `${IMOUMEDIA24_URL}/${path}`;

export {getPosts, getCategories, getAuthor, getImage};
